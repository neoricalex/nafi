...
modelo = Modelo(numero_velas_para_prever, 1500, 3)
neuronio = modelo
saldo = 100
orcamento = 100
...
for tecido in range(0, tamanho_alvo, ignorar):
    acao, comprar = agir(neuronio, estado)
    estado_seguinte = estado_atual(alvo, tecido + 1, numero_velas_para_prever + 1)
    if acao == 1 and orcamento >= alvo[tecido]:
        if comprar < 0:
            comprar = 1
        ...
        total_compra = comprar_unidades * alvo[tecido]
        ...
        total_venda = vender_unidades * alvo[tecido]
